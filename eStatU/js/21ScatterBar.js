// Bargraph matrix
    ...
    var yobs, x1obs, x2obs, x3obs, x4obs, x5obs, tobs, numVar, nrow;
    var freqMaxDM, training, ngroup;
    var fontsize = "1em";
    var rowMax    = 200;
    var colMax    = 6;
    var ngroupMax = 9;
    ...
    var D         = new Array(colMax); // indexed numeric
    var Dtrain    = new Array(colMax); // D copied - indexed numeric
    ...
    for (i = 0; i < colMax; i++) { 
      ...
      d3.select("#executeDT").on("click", function(){  
          ...
          // x1 var statistics
          if (x1obs > 0) {
            if (yobs != x1obs ) { // data size should be the same
              chart.append("text").attr("class","mean").attr("x", 250).attr("y", margin.top + 40)
      ...
      // cross table matrix
      d3.select("#crossTable").on("click", function() {
          crossTable(numVar);
      })
